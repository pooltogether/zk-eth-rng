...
contract RANDAOOracleTest is Test {
  ...
  function testConstructorPoke() public {
    vm.roll(100);
    uint256 pokedRANDAO = block.prevrandao;
  ...
  function testCannotRequestRandomnessForCurrentOrPastBlock(uint256 blockNum) public {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(RANDAOProvider.RequestedRandomnessFromPast.selector, 200)
    );
  ...
  function internalRecoverVerifiedRANDAOValue(uint256 blockNum, bool validBlockhash) internal {
    ...
    if (validBlockhash) {
      ...
      vm.expectRevert(
        abi.encodeWithSelector(RANDAOProvider.BlockhashUnverified.selector, keccak256(rlp))
      );
  ...
  function testCannotFetchRandomnessWithoutValidRANDAOSubmission() public {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(RANDAOProvider.RandomnessNotAvailable.selector, blockNum)
    );
  ...
  function readBlockDataFile(
    uint256 blockNum
  ) public view returns (bytes32 expectedHash, bytes memory rlp, bytes32 mixHash) {
  ...
  function mockOracle(bytes32 blockHash, uint256 response) internal {
    stdstore
      ...
      .sig("blockhashToBlockNum(bytes32)")
      .with_key(blockHash)
